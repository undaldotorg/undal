...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<UndalUnit>("UndalUnit");
#else
    qRegisterMetaType<UndalUnit>("UndalUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. undal_de.qm (shortcut "de" needs to be defined in undal.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. undal_de_DE.qm (shortcut "de_DE" needs to be defined in undal.qrc)
...
static const char* qt_argv = "undal-qt";

UndalApplication::UndalApplication()
...
void UndalApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the UndalApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", UndalGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void UndalApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool UndalApplication::createOptionsModel(bool resetSettings)
{
...
void UndalApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new UndalGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &UndalGUI::quitRequested, this, &UndalApplication::requestShutdown);
...
void UndalApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void UndalApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool UndalApplication::baseInitialize()
{
    return node().baseInitialize();
}

void UndalApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &UndalApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &UndalApplication::handleRunawayException);
    connect(this, &UndalApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &UndalApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void UndalApplication::parameterSetup()
{
...
void UndalApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void UndalApplication::requestInitialize()
{
...
void UndalApplication::requestShutdown()
{
...
void UndalApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // undal: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &UndalGUI::handlePaymentRequest);
            connect(window, &UndalGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void UndalApplication::handleRunawayException(const QString &message)
{
...
void UndalApplication::handleNonFatalException(const QString& message)
{
...
WId UndalApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool UndalApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", UndalGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(undal);
    Q_INIT_RESOURCE(undal_locale);
    ...
    /// 6-7. Parse undal.conf, determine network, switch to network specific
    ...
    // undal: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
