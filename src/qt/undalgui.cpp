...
const std::string UndalGUI::DEFAULT_UIPLATFORM =
...
UndalGUI::UndalGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &UndalGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &UndalGUI::showModalOverlay);
...
void UndalGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Undal address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and undal: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &UndalGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &UndalGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &UndalGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Undal addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Undal addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Undal Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Undal Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a undal: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Undal command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &UndalGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &UndalGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &UndalGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &UndalGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &UndalGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &UndalGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &UndalGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &UndalGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &UndalGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &UndalGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &UndalGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &UndalGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &UndalGUI::showDebugWindow);
}

void UndalGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void UndalGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &UndalGUI::setCurrentWalletBySelectorIndex);
...
void UndalGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &UndalGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &UndalGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &UndalGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &UndalGUI::showProgress);
...
#ifdef ENABLE_WALLET
void UndalGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void UndalGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &UndalGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &UndalGUI::removeWallet);
...
WalletController* UndalGUI::getWalletController()
{
    return m_wallet_controller;
}

void UndalGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &UndalGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &UndalGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &UndalGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &UndalGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &UndalGUI::incomingTransaction);
    connect(this, &UndalGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void UndalGUI::removeWallet(WalletModel* walletModel)
{
...
void UndalGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void UndalGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void UndalGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void UndalGUI::setWalletActionsEnabled(bool enabled)
{
...
void UndalGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &UndalGUI::toggleHidden);
...
void UndalGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void UndalGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void UndalGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void UndalGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void UndalGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void UndalGUI::openClicked()
{
...
void UndalGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void UndalGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void UndalGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void UndalGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void UndalGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void UndalGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void UndalGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void UndalGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Undal network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void UndalGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void UndalGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void UndalGUI::updateHeadersSyncProgressLabel()
{
...
void UndalGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void UndalGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &UndalGUI::quitRequested);
...
void UndalGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void UndalGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void UndalGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &UndalGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &UndalGUI::show);
                e->ignore();
            }
        }
    }
...
void UndalGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void UndalGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void UndalGUI::incomingTransaction(const QString& date, UndalUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(UndalUnits::formatWithUnit(unit, amount, true));
...
void UndalGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void UndalGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool UndalGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool UndalGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void UndalGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void UndalGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void UndalGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void UndalGUI::updateWindowTitle()
{
...
void UndalGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void UndalGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void UndalGUI::detectShutdown()
{
...
void UndalGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void UndalGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(UndalGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = UndalGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void UndalGUI::subscribeToCoreSignals()
{
...
void UndalGUI::unsubscribeFromCoreSignals()
{
...
bool UndalGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<UndalUnit> units = UndalUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const UndalUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, UndalUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const UndalUnit u : UndalUnits::availableUnits()) {
        menu->addAction(UndalUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(UndalUnit newUnits)
{
    setText(UndalUnits::longName(newUnits));
}
