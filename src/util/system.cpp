...
const char * const UNDAL_CONF_FILENAME = "undal.conf";
const char * const UNDAL_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //undal-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", UNDAL_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Undal
    // macOS: ~/Library/Application Support/Undal
    // Unix-like: ~/.undal
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Undal";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Undal";
#else
    // Unix-like
    return pathRet / ".undal";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", UNDAL_CONF_FILENAME));
}
