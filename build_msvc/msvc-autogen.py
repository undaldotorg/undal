...
def parse_config_into_ubtc_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Undal Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/undal_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/undal_config.h'), "w", encoding="utf8") as ubtc_config:
        ubtc_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Undal-core msbuild configuration initialiser.')
    ...
    parse_config_into_ubtc_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/undal_config.h'), os.path.join(SOURCE_DIR, 'config/undal-config.h'))
