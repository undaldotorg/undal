...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing undal-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='undal-cli', help='undal-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Undal address to which the faucet should send')
...
parser.add_argument('undal_cli_args', nargs='*', help='Arguments to pass on to undal-cli (default: -signet)')

args = parser.parse_args()

if args.undal_cli_args == []:
    args.undal_cli_args = ['-signet']


def undal_cli(rpc_command_and_params):
    argv = [args.cmd] + args.undal_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = undal_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = undal_cli(['getnewaddress', 'faucet', 'bech32'])
