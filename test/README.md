This directory contains integration tests that test undald and its
...
undald and undal-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (undal-util, undal-tx, ...).
...
Before tests can be run locally, Undal Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the undald nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another undald
process running on the system (perhaps from a previous test which hasn't successfully
killed all its undald nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other undald processes are running.

On linux, the test framework will warn if there is another
undald process running when the tests are started.

If there are zombie undald processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
undald processes running on the system, so should not be used if any non-test
undald processes are being run.**
...
sure undald processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and undald `debug.log`s will all be dumped
...
`test_framework.log` and undald `debug.log`s can be combined into a single
...
call methods that interact with the undald nodes-under-test.

If further introspection of the undald instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/undald.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/undald <pid>
```
