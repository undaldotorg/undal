Undal Core version 0.11.0 is now available from:

  <https://undal.org/bin/undal-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer UBTC/kB fee to be
...
For example, add the following to `undal.conf`:
...
related to the blockchain in the undal system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Undal Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Undal Core will 
...
For an example script see [undal-submittx](https://github.com/laanwj/undal-submittx).
...
- #5334 `cf87536` libundalconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new undal-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only UndalAmountField
- #5753 `0f44672` Add undal logo to about screen
...
- #5809 `46bfbe7` Add undal-cli man page
...
- #5849 `41259ca` contrib/init/undald.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Undal-Qt.app to Undal-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for undal-tx)
