# CJDNS support in Undal Core

It is possible to run Undal Core over CJDNS, an encrypted IPv6 network that
...
done outside of Undal Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Undal network and individual nodes.
...
before it will work with your Undal Core node. This is described in steps
...
## Run Undal Core with CJDNS

Once you are connected to the CJDNS network, the following Undal Core
...
When enabled, this option tells Undal Core that it is running in an
...
IPv6 local network. This helps Undal Core perform better address management:
...
CJDNS support was added to Undal Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `undal-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/undal/undal/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Undal Core

There are several ways to see your CJDNS address in Undal Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `undal-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `undal-cli getpeerinfo`).
