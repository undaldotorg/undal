# PSBT Howto for Undal Core

Since Undal Core 0.17, an RPC interface exists for Partially Signed Undal
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/undal/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Undal transactions that are not fully signed
...
#### Multisig with multiple Undal Core instances
...
Undal Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `undal-cli` is used.
...
Later, when *V* UBTC has been received on *Amulti*, and Bob and Carol want to
